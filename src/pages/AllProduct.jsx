...
const AllProduct = () => {
  ...
  var filterData=null
  {
    filter===null?
    filterData=products:
    filterData=products.filter(product=>product.category.slug===filter)
  }
  ...
  return (
    ...
    <div className="allproduct  text-center my-5 Container ">
      ...
      {categories.map((item)=>
      <button key={item._id} className={`${item.slug===filter?'active':''} py-2 px-4`} onClick={()=>setFilter(item.slug)}>{item.slug}</button>
    )}
    </div>
    {filterData.length?
    <>
    <h2 className="my-5"> {filterData===products?'All Product':`${filter}`}</h2>
...

  {
    filterData.map((item)=>
        ...
        <Card.Body className="d-flex flex-column justify-content-between">
          <Card.Title className="fs-5 my-3">{item.title}</Card.Title>
          <div>
          ...
          <Button variant="none" className="py-2 d-flex justify-content-center mx-auto my-3 w-75 text-capitalize" onClick={()=>console.log(item._id)}>add to cart</Button>
          </div>
        </Card.Body>
      </Card>
  )
