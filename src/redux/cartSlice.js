...
export const getUserCart=createAsyncThunk('cart/getUserCart',
    async(_,{getState})=>{
        const token=getState().user.userData.token
...
export const remove=createAsyncThunk('cart/remove',
    async(_, {getState})=>{
        const token=getState().user.userData.token
        const id =getState().products.productId
        console.log(id)
...
export const{productCart,changeCount}=cartSlice.actions